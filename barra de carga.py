...
'''
import time
# Solicitar al usuario el tamaño del archivo y el tiempo de carga.
tamaño_archivo = float(input("Ingrese el tamaño del archivo (MB): "))
tiempo = float(input("Ingrese el tiempo de carga (segundos): "))
# Calcular el tiempo entre actualizaciones (usando intervalos de 5%).
a = tiempo/ 20  # 20 actualizaciones para llegar al
# 100%
print(f"Iniciando subida de {tamaño_archivo} MB...")
for i in range(21):  # De 0% a 100% en incrementos de 5%
    porcentaje = i * 5
    # Crear la barra de progreso.
    barra = "[" + "#" * i + "-" * (20 - i) + "]"
    # Mostrar la barra y el porcentaje.
    print(f"\r{barra} {porcentaje}%", end="")
    time.sleep(a)  # Pausa para simular el tiempo de carga 
print(f"\n\n¡Archivo de {tamaño_archivo} MB subido con éxito!") 
